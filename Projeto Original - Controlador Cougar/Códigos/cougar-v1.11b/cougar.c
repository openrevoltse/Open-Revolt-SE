...
version 1.11c - Fixed but where high pedal lockout could come off while precharging.  
		When throttle hits 0, wait 10 sec, then bring B4 low.
		When throttle != 0, bring B4 high.
		If throttle == 0, and current != 0, open main contactor.
...
typedef struct {
	...
	unsigned throttle_min_raw_counts;	// throttle low voltage (Zero Throttle)
	unsigned throttle_max_raw_counts;	// throttle high voltage (Max throttle)
	unsigned throttle_fault_min_raw_counts;	// throttle fault low voltage (after 200mS)
	unsigned throttle_fault_max_raw_counts; // throttle fault high voltage (after 200mS)

	unsigned max_rpm;				// Any rpm above this will set a fault
...
config_type default_config PROGMEM = {
	...
	164,								// throttle low voltage (foot off pedal)
	839,								// throttle high voltage (full throttle)
	102,								// throttle fault min voltage (after 200mS)
	922,								// throttle fault MAX voltage (after 200mS)
	3300,								// the max rpm
...
unsigned char tach_input_low = 0;		// to remember last tach input state
...
volatile unsigned throttleZeroCounter = 0;	// How long as throttle been 0?  Units in milliSec.
volatile unsigned ocr1a_ghost = 0;		// ocr1a ghost variable (needed for 8KHz PWM)

volatile unsigned tach_ticks = 0;		// tach ticks accumulator
volatile unsigned tach_ticks_pulse = 0;	// tach ticks per pulse
...
unsigned long tach_tpm;					// ticks per minute for tachometer
...
void pi_loop(void)
{
	...
	// current starts in [512, 512 + 298] (if it's in 0 to 700 amps for the LEM 300)
	if (loc_current_fb < vref) loc_current_fb = 0;
	else loc_current_fb -= vref;
	// now current is in the range [0, 298] or so
        // or current is in the range [0,213] (if it's in 0 to 500amp for the LEM 300)
	...
	if ((throttle_counter & 0x03) == 0x00) {
		...
		if (loc_throttle >= config.throttle_fault_min_raw_counts && loc_throttle <= config.throttle_fault_max_raw_counts) {
			// throttle is OK
			...
			// NO NEED TO INVERT IT, SINCE DAMIEN IS USING A HALL EFFECT SENSOR NOW.  So comment this out!
		...
		//		loc_throttle;
			...
			*/
			...
			// NEW STUFF FOR DAMIEN
			if (throttle_ref <= 20 && (PORTB & (1 << PB4))) {
				PORTB &= ~(1 << PB4); // turn the contactor off
			}
			else if (fault_bits == 0 && throttle_ref >= 30) {  // If the High pedal lockout is over and done with,  								       // throttle_ref >= 30, go ahead and turn on damien's shunt contact
				if (!(PORTB & (1 << PB4))) { // If the contactor is off, then turn it on.
					PORTB |= (1 << PB4);
				}
			}
			// END NEW STUFF FOR DAMIEN
		}
		else {
		...
		// run battery amps and hours logic and rpm overlimit fault at 1KHz.  Make sure the rpm has been over the limit for at least 100mS
		...
		if (rt_data.tach_rpm < config.max_rpm - 100) {
			// rpm < fault value - rpm is OK.
			if (rpm_fault_counts > 0) rpm_fault_counts--;
			if (rpm_fault_counts == 0)  // clear the fault
				fault_bits &= ~RPM_OVERLIMIT_FAULT;
		}
		else if (rt_data.tach_rpm > config.max_rpm) {
			// rpm > fault value - it this persists we will have an rpm fault
			if (rpm_fault_counts < RPM_FAULT_COUNTS) {
				rpm_fault_counts++;
				if (rpm_fault_counts >= RPM_FAULT_COUNTS) fault_bits |= RPM_OVERLIMIT_FAULT;
			}
		}
	}
...
void config_tach(void)
{
	...
	tach_tpm = 16000L * 60L;
	#endif
	// tach_tpm is ticks per minute used to measure RPM
	// for sensors that pulse more than once per revolution, simply divide by PPR to get the correct RPM
	tach_tpm = tach_tpm / (unsigned long)TACH_PULSES_PER_REV;
}

void fetch_rt_data(void)
{
	unsigned ticks, ticks_pulse;
	unsigned long rpm;
	...
	// fetch tachometer data
	cli(); ticks = tach_ticks; ticks_pulse = tach_ticks_pulse; sei();
	#ifdef PWM8K
	if (ticks >= 8000) {
	#else
	if (ticks >= 16000) {
	#endif
		// no pulse for more a second or more - consider RPM to be zero
		rt_data.tach_rpm = 0;
	}
	else {
		if (ticks_pulse == 0) ticks_pulse = 1;
		rpm = tach_tpm / (unsigned long)ticks_pulse;	// tach_tpm is a constant.
		
		if (rpm > 9999) rt_data.tach_rpm = 9999;
		else rt_data.tach_rpm = rpm;
	}
}
...
{
	#ifdef PWM8K
	strcpy_P(uart_str, PSTR("Cougar OS controller firmware v1.11c-rpm (8KPWM)\r\n"));
	#else
	strcpy_P(uart_str, PSTR("Cougar OS controller firmware v1.11c-rpm\r\n"));
	#endif
	uart_putstr();
}
...
void show_config(unsigned mask)
{
	...
	if (mask & ((unsigned)1 << 2)) {
		strcpy_P(uart_str, PSTR("throttle_fault_min_raw_counts=xxxx throttle_fault_max_raw_counts=xxxx\r\n"));
		u16_to_str(&uart_str[30], config.throttle_fault_min_raw_counts, 4);
		u16_to_str(&uart_str[65], config.throttle_fault_max_raw_counts, 4);
		uart_putstr();
	}
	...
	if (mask & ((unsigned)1 << 15)) {
		strcpy_P(uart_str, PSTR("max_rpm=xxxx\r\n"));
		u16_to_str(&uart_str[8], config.max_rpm, 4);
		uart_putstr();
	}
...
void process_command(char *cmd, int x)
{
	if (!strcmp_P(cmd, PSTR("config"))) {
		...
		u16_to_str(&uart_str[4],
			(unsigned)(wait_time(100) * (unsigned long)100 / idle_loopcount), 3);		
		uart_putstr();
	}
	...
	else if (!strcmp_P(cmd, PSTR("t-fault-min-rc"))) {
		if ((unsigned)x <= 1023) {
			cli(); config.throttle_fault_min_raw_counts = x; sei();
			show_config((unsigned)1 << 2);
		}
	}
	else if (!strcmp_P(cmd, PSTR("t-fault-max-rc"))) {
		if ((unsigned)x <= 1023) {
			cli(); config.throttle_fault_max_raw_counts = x; sei();
			show_config((unsigned)1 << 2);
		}
	}
	...
	else if (!strcmp_P(cmd, PSTR("max-rpm"))) {
		if ((unsigned)x <= 9998 && (unsigned)x > 100) {
			config.max_rpm = x;
			show_config((unsigned)1 << 15);
		}
	}
...
int main(void)
{
	...
	PORTB = 0xff & ~PB_PWM & ~(1 << PB4);					// PWM output low, pb4 low, other outputs high, weak pullups on
	...
	config_tach();							// configure tach from config structure
	...
	while (1) {
		...
		if (config.rtd_period) {
			if (diff_time(tm_show_data) >= config.rtd_period) {
				...
				strcpy_P(uart_str, PSTR("TR=xxx CR=xxx CF=xxx PW=xxx HS=xxxx RT=xxxx FB=xx BA=xxx AH=xxx.x RM=xxxx\r\n"));
				...
				u16_to_str(&uart_str[69], rt_data.tach_rpm, 4);
				uart_putstr();
			}
		}
		if (diff_time(tm_100) >= 100) {
			...
			if (fault_bits) {
				...
				if (fault_bits & HPL_FAULT) {
					...
					if (rt_data.throttle_ref <= HPL_THROTTLE_THRESHOLD && precharge_timer == 0) {
